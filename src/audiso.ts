  ...
  result &&=
    ...
    Object.entries(data.metadata.vulnerabilities).every(([key, value]) => {
      ...
      if (!isValid) {
        LOGGER.error(
          LoggerTheme.DATA,
          "Invalid metadata of vulnerability object",
          Error(
            `Invalid metadata ${key}, name of metadata is typeof ${typeof key}, value is ${value} of type ${typeof value}`
          )
        );
      }
      return isValid;
    ...
    Object.entries(data.vulnerabilities).every(([key, value]) => {
      ...
      if (!isValid) {
        LOGGER.error(
          ...
          Error(
            `Invalid vulnerability ${key}, name of vulnerability is typeof ${typeof key}, vulnerability check result ${isVulnerabilityCheck}`
          )
        );
      }
      return isValid;
...
function isVulnerability(data: any): data is Vulnerability {
  let result = true;
  result &&=
    typeof data?.fixAvailable === "boolean" ||
  ...
  if (!result) {
    LOGGER.error(
      LoggerTheme.DATA,
      "Bad vulnerability object definition",
      Error(
        `is not a valid vulnerability object, data fixAvailable of ${data?.fixAvailable}, is direct ${data?.isDirect}, severity ${data?.severity}`
      )
    );
  }
  return result;
...
async function readAuditEntry(inputFilePath : string): Promise<Audit> {
  ...
  try {
    ...
    const rethrowError = Error("entry data invalid, parsing error");
    LOGGER.error(
      LoggerTheme.DATA,
      "entry data invalid, parsing error",
      error instanceof Error ? error : rethrowError
    );
    throw rethrowError;
  }
...
async function auditToSonar(params: Parameters): Promise<void> {
  ...
  for (const [packageName, vulnerability] of Object.entries(
    ...
    if (vulnerability.isDirect) {
      const packageJsonFileBuffer = await fs.readFile(params.packageFilePath);
  ...
  LOGGER.debug(LoggerTheme.DATA, "issues generated", output);
  try {
    await fs.writeFile(params.outputFilePath, output);
  } catch (error) {
    const rethrowError = Error("output error, report not writed");
    LOGGER.error(
      LoggerTheme.IO,
  ...
  (async function () {
    ...
    try {
      ...
      if (params !== null) {
        ...
        const error = Error("entry parameters invalid");
        LOGGER.error(LoggerTheme.IO, "parameters invalid", error);
        throw error;
      }
    } catch (error) {
        // result = 1;
