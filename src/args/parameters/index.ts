...
export function isParameters(data: any): data is Parameters {
  return (
    typeof data === "object" &&
    [data?.inputFilePath, data?.outputFilePath, data?.packageFilePath].every(
...
export async function readParameters(
  ...
  try {
    ...
    configuration.forEach((configurationItem: ConfigurationItem) => {
      ...
      if (
        ["inputFilePath", "outputFilePath", "packageFilePath"].includes(key)
      ) {
        extractedParams[key] =
          typeof value === "string" ? path.resolve(process.cwd(), value) : "";
      }
    });
    ...
    const configurationMiss = Error(
      "configuration core exploitation raise error"
    );
    LOGGER.error(
      LoggerTheme.DATA,
      "configuration core exploitation raise error",
      error instanceof Error ? error : configurationMiss
    );
  }
  return params;
}
