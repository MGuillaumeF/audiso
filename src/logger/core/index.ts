...
export abstract class Logger {
  ...
  public trace(
    ...
    if (level >= this.configuration[theme].level) {
      ...
      this.configuration[theme].appenders.forEach((appender) =>
        appender(message)
      );
    }
  }
...
export class CliLogger extends Logger {
  ...
  constructor() {
    ...
    this.configuration = {
      IO: {
        ...
        appenders: [console.info, this.write]
      },
      DATA: {
        level: ELoggerLevel.INFO,
        appenders: [console.info, this.write]
  ...
  private write(message: string): void {
    if (CliLogger.batchLog.length < MAX_COL) {
      ...
      fs.writeFile(
        path.resolve(process.cwd(), "audiso.log"),
        `${CliLogger.batchLog.join("\n")}\n`,
        { flag: "a" }
      );
      CliLogger.batchLog = [];
    }
  ...
  public getMessage(
    ...
    return `${new Date().toLocaleString("fr-FR")} [${MLogLevel.get(
      level
    )}] - ${theme} : ${messages.join(" ")} ${error?.message} - stack : ${
      error?.stack
    }`;
  }
